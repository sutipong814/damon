...
settings = {
    ...
    "message":"บัญชีนี้ได้การป้องกันโดย\n Ŧ€₳M↬฿❂Ŧ↬ȾɆ₦₮Ɑ₡ŁɆ🇹🇭  \nระบบได้ทำการบล็อคคุณอัตโนมัติ",
...
🌠เช็คแทค 👉 เช็คแทค
🌠เปิดเตะ 👉 เปิดแจ้งเตือนคนเตะ
🌠ปิดเตะ 👉 ปิดแจ้งเตือนคนเตะ
...
☃เขียน ข้อความ ➠ เขียน
☃รูปมา ข้อความ ➠ ค้นหารูป
☃google ข้อความ ➠ ผลค้นหา
☃เพลง ข้อความ ➠ ค้นหาเพลง
☃กาตูน ข้อความ ➠ ลงรูป"""
    return helpKicker
...
# ===========
                ...
                elif "กลอนเอ๊ะ" == msg.text.lower():
                        line.sendMessage(to,"เหมือนฟ้าร้อง ฟ้าผ่า น่าตกใจ\nต้องจากไป ไกลตา พาแลมอง\nเพื่อนฝูง ทุกหมู่เหล่า ต่างคับข้อง\nทำไมต้องโสด แบบหน้าตาดี อย่างนี้\n\nการจากไป กลายเป็นอื่น เราขื่นขม\nเราเป็นคน ที่เหมาะสม คนค้ำคูณ\nพัฒนา ครอบครัว อย่างเกื้อกูล\nบูรณะ บ้านเรือน คูณความดี\n\nข้าพเจ้า นายสุธิพงษ์  คนซื่อตรง\nเป็นคน  มัญจาคีรี คนบ้านนี้\nมุ่งมั่น ที่กระทำ แต่สี่งดี\nให้บ้านเรือน สุขเปรมปรีดิ์ มีโชคชัย\n\nมีความสุข อบอุ่น ไปที่อื่น\nให้สดชื่น รื่นเริง ตามสมัย\nขอให้เรา โชคดี และมีชัย\nทำสี่งใด สำเร็จ ทุกเม็ดงาน..เอ่ย")
# ==============================================================================#
...
#==============================================================================#   
                    ...
                    line.sendMessage(msg.to, t1 + txt + t2)						
                        ...
                        line.sendMessage(msg.to,"❋ส่ง คท. คนที่ต้องการดึง ❋send a contact to invite user❋")                            
...
#=============COMMAND KICKER===========================#
                ...
                elif "ล้างหี" in msg.text:
                	if msg.toType == 2:
                         _name = msg.text.replace("ล้างหี","")
...
#-------------------------------------------------------------------------------
                    ...
                    if msg._from in bot1:
                        settings["blacklist"] = {}
                        line.sendMessage(msg.to,"เคลียร์บัญชีดำทั้งหมดเรียบร้อยแล้ว❋ Blacklist Dibersihkan❋")
...
#=======================================================================================
                ...
                elif msg.text in ["เตะแบน","Scan blacklist"]:
                	if msg.toType == 2:
                         ...
                         if matched_list == []:
                             line.sendMessage(receiver,"ไม่พบคนติดดำในกลุ่มนี้ ❋ Nots in Blacklist")
                         else:
...
#=============COMMAND PROTECT=========================#
                    ...
                    if RfuProtect["Protectguest"] == True:
                        if settings["lang"] == "JP":
                            line.sendMessage(msg.to,"เปิดป้องกันสมาชิกในกลุ่ม   ")
                        else:
                            line.sendMessage(msg.to,"เปิดป้องกันสมาชิกในกลุ่ม   ")
                    else:
                    ...
                    if RfuProtect["Protectguest"] == False:
                        if settings["lang"] == "JP":
                            line.sendMessage(msg.to,"ปิดป้องกันสมาชิกในกลุ่ม   ")
                        else:
                            line.sendMessage(msg.to,"ปิดป้องกันสมาชิกในกลุ่ม   ")
                    else:
...
#-------------------------------------------------------------------------------
                    ...
                    if msg._from in bot1:
                        if settings["blacklist"] == {}:
                            line.sendMessage(msg.to,"ไม่มีผู้ใช้ที่ติดดำ ❋ Tidak Ada Banlist")
                        else:
                            line.sendMessage(msg.to,"รายชื่อผู้ใช้ที่ติดดำ ❋ Daftar Banlist")
                            num=1
                            msgs="══════════List Blacklist═════════"
                            for mi_d in settings["blacklist"]:
                                msgs+="\n[%i] %s" % (num, line.getContact(mi_d).displayName)
...
#==============================================================================#
        ...
        if op.type == 26:
            ...
            if msg.toType == 0:
                ...
                if msg.text in ["แทค","tagall","Tagall","tag"]:
                    line.sendText(msg.to,"❋ขออนุญาตแทคนะครับ❋")				
        ...
        if op.type == 19:
          ...
          if settings["Nk"] == True:
            ...
            line.sendMessage(op.param1,str(settings["kick"]) + "\nเฮ้ย..ป๊าด {}, คือหยังมันโหดแท้วะΣ(っﾟДﾟ；)っ ".format(str(dan.displayName)))
            line.sendContact(op.param1, op.param2)
